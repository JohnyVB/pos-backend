...
exports.getCashBoxSessions = exports.closeCashBoxSession = exports.openCashBoxSession = void 0;
const postgresql_config_1 = require("../config/postgresql.config");
const openCashBoxSession = async (req, res) => {
    const { opening_amount, pos_terminal_id } = req.body;
    const userId = req.user?.id;
    const { store_id } = req.params;
    try {
        const result = await postgresql_config_1.pool.query(`INSERT INTO cash_box_sessions (opened_at, opening_amount, user_id, pos_terminal_id, status, store_id) 
...
exports.openCashBoxSession = openCashBoxSession;
const closeCashBoxSession = async (req, res) => {
    const { cash_box_id, closing_amount } = req.body;
    try {
        const result = await postgresql_config_1.pool.query("UPDATE cash_box_sessions SET closed_at=NOW(), closing_amount=$1, status='CLOSED' WHERE id=$2 RETURNING id as session_id, *", [closing_amount, cash_box_id]);
        res.status(200).json({ response: "success", cashBoxSession: result.rows[0] });
    }
    catch (err) {
        console.log(err);
...
exports.closeCashBoxSession = closeCashBoxSession;
const getCashBoxSessions = async (req, res) => {
    const user = req.user; // Datos del token JWT (id, role, store_id)
    const { store_id: paramStoreId } = req.params;
...
exports.getCashBoxSessions = getCashBoxSessions;
