...
exports.processRefund = exports.getSalesBySessionId = exports.createSale = void 0;
const postgresql_config_1 = require("../config/postgresql.config");
const UpdateInventory_helper_1 = require("../helper/UpdateInventory.helper");
const UpdateSaleItems_helper_1 = require("../helper/UpdateSaleItems.helper");
const createSale = async (req, res) => {
    const { store_id } = req.params;
    const user_id = req.user?.id;
    const client = await postgresql_config_1.pool.connect();
    try {
        ...
        WHERE p.id = $2`, [promo_id, product_id]);
            ...
            await client.query("UPDATE inventory SET quantity = quantity - $1 WHERE product_id = $2 AND store_id = $3", [quantity, product_id, store_id]);
            // 4. Registrar movimiento
            await client.query(`INSERT INTO inventory_movements (product_id, movement_type, quantity, reference, created_at, store_id) 
         VALUES ($1, 'SALE', $2, $3, NOW(), $4)`, [product_id, quantity, reference || `SALE_${Date.now()}`, store_id]);
        }
        const change_amount = payment_method === "CASH" ? amount_received - total_final : 0;
        // 5. Registrar venta (Cabecera)
        const saleRes = await client.query(`INSERT INTO sales (
      ...
      RETURNING id`, [user_id, cash_box_id, subtotal_bruto, vat_total_final, total_final, payment_method, amount_received, change_amount, store_id, total_descuentos]);
        const sale_id = saleRes.rows[0].id;
        // 6. Registrar detalle_venta (sale_items)
        for (const pi of processedItems) {
            await client.query(`INSERT INTO sale_items (sale_id, product_id, quantity, price, vat, subtotal, promo_id, discount_applied) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [sale_id, pi.product_id, pi.quantity, pi.price_at_sale, pi.vat_rate, pi.final_subtotal, pi.promo_id, pi.discount]);
        }
...
const getSalesBySessionId = async (req, res) => {
...
exports.getSalesBySessionId = getSalesBySessionId;
const processRefund = async (req, res) => {
    const { store_id } = req.params;
    const { sale_id, session_id, user_id, reason, items } = req.body;
    const total_refunded = items.reduce((acc, item) => {
    ...
    `;
        ...
        await (0, UpdateInventory_helper_1.UpdateInventory)(items, "add", postgresql_config_1.pool);
        await (0, UpdateSaleItems_helper_1.UpdateSaleItems)(items, sale_id, postgresql_config_1.pool);
        const saleRes = await client.query("SELECT (CASE WHEN total = subtotal THEN total + vat_total ELSE total END) as original_total FROM sales WHERE id = $1", [sale_id]);
        const original_total = Number(saleRes.rows[0]?.original_total || 0);
        const allRefundsRes = await client.query("SELECT COALESCE(SUM(total_refunded), 0) as total_refunded_sum FROM refunds WHERE sale_id = $1", [sale_id]);
